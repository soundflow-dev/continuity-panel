...
for item in AGENTS.md LICENSE README.md SECURITY.md install.sh "Install ContinuityPanel.command" assets bin config helpers templates; do
...
case "$MODE" in
  ...
  --verify|verify)
    ...
    ;;
  --package|package)
    ARCHIVE="$DIST_DIR/ContinuityPanel-0.4.0-macos.zip"
    rm -f "$ARCHIVE"
